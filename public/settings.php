...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $submittedApiKey = trim((string) ($_POST['image_search_api_key'] ?? ''));
    $settings['image_search'] = [
        'enabled' => !empty($_POST['image_search_enabled']),
        'api_key' => $submittedApiKey !== '' ? $submittedApiKey : (string) ($settings['image_search']['api_key'] ?? ''),
        'cse_id' => trim((string) ($_POST['image_search_cse_id'] ?? '')),
    ];
    ...
    <form method="post" class="stacked-form"<?= $editMode ? ' data-edit-form' : ''; ?>>
        ...
        <?php if (!$editMode): ?>
            <div class="detail-sections">
                ...
                <section class="detail-section"><h3>Bildersuche</h3><div class="detail-grid"><div class="detail-row"><span class="detail-label">Status</span><span class="detail-value"><?= !empty($settings['image_search']['enabled']) ? 'Aktiviert' : 'Deaktiviert'; ?></span></div><div class="detail-row"><span class="detail-label">API-Key</span><span class="detail-value"><?= ($settings['image_search']['api_key'] ?? '') !== '' ? 'Hinterlegt' : 'Nicht konfiguriert'; ?></span></div><div class="detail-row"><span class="detail-label">Search Engine ID</span><span class="detail-value"><?= e($settings['image_search']['cse_id'] ?? '') ?: 'Nicht konfiguriert'; ?></span></div></div></section>
            </div>
        <?php else: ?>
            <div class="form-sections">
                ...
                <section class="form-section"><h3>Bildersuche (Google Custom Search)</h3><div class="row two-col"><label><input type="checkbox" name="image_search_enabled" value="1" <?= !empty($settings['image_search']['enabled']) ? 'checked' : ''; ?> /> Automatische Artikelbildsuche aktivieren</label></div><div class="row two-col"><label>API-Key<input type="password" name="image_search_api_key" value="" placeholder="<?= ($settings['image_search']['api_key'] ?? '') !== '' ? 'Unverändert lassen' : ''; ?>" autocomplete="off" /></label><label>Search Engine ID (cx)<input type="text" name="image_search_cse_id" value="<?= e($settings['image_search']['cse_id'] ?? ''); ?>" /></label></div><p class="form-hint">Wird genutzt, um anhand von Hersteller und Gerätename automatisch ein Artikelbild zu ermitteln. Leeres API-Key-Feld beim Speichern lässt den bisherigen Key unverändert.</p></section>
            </div>
