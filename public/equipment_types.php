...
<section class="card"><h2>Verfügbare Gerätetypen</h2><div class="table-wrap"><table id="equipment-types-table" data-sortable><thead><tr><th><button type="button" class="sort-button" data-sort="0" data-direction="asc">Gerätetyp</button></th><th><button type="button" class="sort-button" data-sort="1" data-sort-type="number">Verwendung bei Geräten</button></th><th>Aktionen</th></tr></thead><tbody><?php foreach ($types as $type): ?><?php $typeName = (string) ($type['name'] ?? ''); ?><tr><td><?= e($typeName); ?></td><td><?= (int) ($usage[$typeName] ?? 0); ?></td><td class="actions"><a class="button-link" href="/equipment_types.php?edit=<?= (int) ($type['id'] ?? 0); ?>">Bearbeiten</a><form method="post" class="action-form"><input type="hidden" name="action" value="delete" /><input type="hidden" name="id" value="<?= (int) ($type['id'] ?? 0); ?>" /><button type="submit" class="button-muted" <?= (($usage[$typeName] ?? 0) > 0) ? 'disabled title="Noch verwendeter Gerätetyp"' : ''; ?>>Löschen</button></form></td></tr><?php endforeach; ?></tbody></table></div></section>
<?php pageFooter();
