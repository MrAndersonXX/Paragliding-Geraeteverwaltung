...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $settings['regional'] = RegionalSettings::normalize([
        'language' => (string) ($_POST['regional_language'] ?? 'de'),
        'country' => (string) ($_POST['regional_country'] ?? 'DE'),
        'timezone' => (string) ($_POST['timezone'] ?? 'Europe/Berlin'),
        'date_format' => (string) ($_POST['regional_date_format'] ?? 'd.m.Y'),
    ]);
    $settings['mail'] = [
    ...
    <form method="post" class="stacked-form"<?= $editMode ? ' data-edit-form' : ''; ?>>
        ...
        <?php if (!$editMode): ?>
            <div class="detail-sections">
                <section class="detail-section"><h3>Anwendung</h3><div class="detail-grid"><div class="detail-row"><span class="detail-label">Anwendungsname</span><span class="detail-value"><?= e($settings['app']['name'] ?? 'Glider Equipment Tracker'); ?></span></div><div class="detail-row"><span class="detail-label">Sprache</span><span class="detail-value"><?= e(RegionalSettings::languages()[$settings['regional']['language'] ?? 'de']['label'] ?? 'Deutsch'); ?></span></div><div class="detail-row"><span class="detail-label">Land</span><span class="detail-value"><?= e(RegionalSettings::countries()[$settings['regional']['country'] ?? 'DE'] ?? 'Deutschland'); ?></span></div><div class="detail-row"><span class="detail-label">Zeitzone</span><span class="detail-value"><?= e($settings['regional']['timezone'] ?? ($settings['app']['timezone'] ?? 'Europe/Berlin')); ?></span></div><div class="detail-row"><span class="detail-label">Datumsformat</span><span class="detail-value"><?= e(RegionalSettings::dateFormats()[$settings['regional']['date_format'] ?? 'd.m.Y'] ?? '31.12.2026'); ?></span></div></div></section>
        ...
        <?php else: ?>
            <div class="form-sections">
                <section class="form-section"><h3>Anwendung</h3><div class="row two-col"><label>Anwendungsname<input type="text" name="app_name" value="<?= e($settings['app']['name'] ?? 'Glider Equipment Tracker'); ?>" required /></label><label>Sprache<select name="regional_language" required><?php foreach (RegionalSettings::languages() as $language => $languageInfo): ?><option value="<?= e($language); ?>" <?= (($settings['regional']['language'] ?? 'de') === $language) ? 'selected' : ''; ?>><?= e($languageInfo['label']); ?></option><?php endforeach; ?></select></label></div><div class="row three-col"><label>Land<select name="regional_country" required><?php foreach (RegionalSettings::countries() as $country => $label): ?><option value="<?= e($country); ?>" <?= (($settings['regional']['country'] ?? 'DE') === $country) ? 'selected' : ''; ?>><?= e($label); ?></option><?php endforeach; ?></select></label><label>Zeitzone<select name="timezone" required><?php foreach (RegionalSettings::timezones() as $timezone => $label): ?><option value="<?= e($timezone); ?>" <?= (($settings['regional']['timezone'] ?? ($settings['app']['timezone'] ?? 'Europe/Berlin')) === $timezone) ? 'selected' : ''; ?>><?= e($label); ?></option><?php endforeach; ?></select></label><label>Datumsformat<select name="regional_date_format" required><?php foreach (RegionalSettings::dateFormats() as $dateFormat => $label): ?><option value="<?= e($dateFormat); ?>" <?= (($settings['regional']['date_format'] ?? 'd.m.Y') === $dateFormat) ? 'selected' : ''; ?>><?= e($label); ?></option><?php endforeach; ?></select></label></div></section>
